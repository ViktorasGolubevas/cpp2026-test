...
=== "🔨 = ⚙️➔❌"
    ...
    ```
    ```
    user.c:5:5: warning: implicit declaration of function 'init'
    user.c:6:5: warning: implicit declaration of function 'push'
    user.c:11:15: warning: implicit declaration of function 'pop'
    ```
    ⚠️ implicit declaration warnings

---

#### ⚠️ Kompiliuojasi su perspėjimais

??? bug "🔍 Diagnozė: Trūkstamos funkcijų deklaracijos"
    **Problema:** Kompiliatorius nežino `init()`, `push()`, `pop()` funkcijų!
    ...
    **Kodėl:**
    ...
    - **Kompiliatorius** neturi funkcijų **deklaracijų** (prototipų)
    - **Implicit declaration** - kompiliatorius "spėja" `int funkcija()`
    - **Gali veikti** bet tik atsitiktinai - **dangerous**!
    
    **Sprendimas:** Pridėti funkcijų prototipus `user.c` pradžioje arba...

---

### 2 sprendimas: `#include "stack.c"`
...
!!! quote "sumanymas/ketinimas"
    Pridėsiu `#include "stack.c"` - taip user.c "pamatys" visas funkcijas!
...
??? bug "🔍 Diagnozė: Daugybiniai apibrėžimai"
    **Problema:** `#include "stack.c"` įterpia turinį → visi `stack.c` **apibrėžimai** (kintamieji ir funkcijos) po kompiliavimo yra **abiejuose** `.o` failuose.
    
    **Kodėl:**
    ```c
    // user.c po preprocessor:
    ...
    - **Linkeris** turi sulinkinti `.o` failuose esančius kreipinius į vardus su jų **apibrėžimais**
    - Bet apibrėžimų **po du** per abu failus!
    - **Linkeris nežino** kurį pasirinkti → klaida "multiple definition"
    
    **Kodėl negalima automatiškai pasirinkti?** Jei du skirtingi apibrėžimai - kuris teisingas? Linkeris šį sprendimą **deleguoja autoriui**.
    
    **Sprendimas:** `#include` skirtas `.h` failams, ne `.c`!
...
??? warning "🔍 Kodėl BLOGAI (antipattern)?"
    **Veikia, bet grįžome prie monolito problemos:**
    ...
    - **`#include` skirtas `.h`, ne `.c`** - prieš konvenciją
    - **Kompiliavimo laikas** - `stack.c` kompiliuojamas kiekvieną kartą
    
    **Teisingas kelias:** Reikia **header failų** (`.h`)!
...
!!! tip "Veikiantis Kodas ≠ Geras Kodas"
    - **Kompiliavimo klaidos** moko funkcijų deklaracijų
    - **Linkavimo klaidos** moko multiple definition problemos
    - **`#include "file.c"`** - antipattern, nors ir veikia
    - **Sprendimas → Etapas 03:** Tikras C modulis su `.h` failais
