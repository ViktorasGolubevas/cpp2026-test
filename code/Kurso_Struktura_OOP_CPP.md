...
**Lygis:** 1 kursas, Informatika (Computer Science)  
**Paskaitų skaičius:** 15  
**Lygiagretus kursas:** Duomenų struktūros ir algoritmai (DSA)
...
### DALIS I: EVOLIUCIJA
**Tikslas:** Parodyti kodėl C++ atsirado

#### 1.1. Monolitinis → Modulinis (Stages 1-3)
- Programos dekompozicija
- Kompiliavimo vienetai (`compilation units`)
- Antraštės/implementacijos atskyrimas (`header`/`implementation`)

#### 1.2. Informacijos slėpimo pradžia (Stages 4-5)
- Statiniai kintamieji (`static`)
- Vartotojo apibrėžti tipai (`User-Defined Types`, `UDT`)
- Inkapsuliacija (`encapsulation`) C būdu

#### 1.3. Dinaminis gyvavimo ciklas (Stages 6-9)
- Nepermatomi rodykliai (`opaque pointers`)
- Gamyklos šablonas (`factory pattern`)
- Rankinis resursų valdymas
- Gyvavimo ciklo užbaigimo problema (`lifecycle completion`)

#### 1.4. Pažangios C technikos (Priedas 10A-D)
...
### DALIS II: KLASĖS ANATOMIJA
**Tikslas:** Suprasti kas yra objektas

#### 2.1. KLASĖS pagrindai
- `struct` vs `class`
- Narių funkcijos (`member functions`)
- `this` rodyklė
- Konstruktoriai (`constructors`): numatytasis (`default`), parametrizuotas
...
### DALIS III: GERESNIS C
**Tikslas:** Modernesnė sintaksė
...
### DALIS IV: KOPIJAVIMO SEMANTIKA
**Tikslas:** Giliojo kopijavimo problema
...
#### 4.4. Trijų taisyklė (`Rule of Three`)
- Destruktorius + Kopijavimo konstruktorius + Kopijavimo priskyrimas
- Kada reikalinga

---

### DALIS V: OOP FUNDAMENTAI (4 Ramsčiai)
...
### DALIS VI: MODERNUSIS C++ (C++11/14/17)
**Tikslas:** Perkėlimo semantika ir protingi rodykliai
...
- Perkelk-ir-keisk šablonas (`move-and-swap pattern`)

#### 6.4. Penkių taisyklė (`Rule of Five`)
- Trijų taisyklė + Perkėlimo operacijos
- Nulio taisyklė (`Rule of Zero`) - kada nereikia nieko

#### 6.5. Protingi rodykliai (`Smart Pointers`)
...
### DALIS VII: OPERATORIŲ PERKROVIMAS
**Tikslas:** Vartotojo tipai kaip įtaisytieji
...
### DALIS VIII: ŠABLONŲ PAGRINDAI
**Tikslas:** Bendrinis programavimas (`Generic programming`)
...
#### 8.3. Šablonų apribojimai
- `Concepts` (C++20 preview)
- Kada šablonas "veikia"

---

### DALIS IX: STL PAGRINDAI
**Tikslas:** Nenaudoti žalių rodyklių/masyvų (`raw pointers`/`arrays`)
...
### DALIS X: IŠIMČIŲ APDOROJIMAS
**Tikslas:** Klaidų apdorojimas be grąžinamų kodų
...
## PRIEDAS: PAŽANGIOS C TECHNIKOS (10A-10D)

**Statusas:** Self-study medžiaga, ne lecture'uose  
**Tikslas:** Parodyti kad C "galėjo viską", bet rankomis. C++ automatizuoja.  
**Objektas:** Stack (abstraktesnis, language-independent)

### 10A: Typedef Aliasing
- C: `typedef struct Stack* StackHandle;`
- C++: `using StackHandle = std::unique_ptr<Stack>;`

### 10B: Const Correctness
- C: `const Stack* s` vs `Stack* const s`
- C++: `const Stack&`, `const` methods

### 10C: Function Pointers (Polymorphism)
- C: `int (*compare)(void*, void*)`
- C++: `virtual functions`, `std::function`, `lambdas`

### 10D: Void Pointer Generics
- C: `void* data` + type tracking
- C++: `Templates` `Stack<T>`

---

## P2 PABAIGOS WOW MOMENTAS

```cpp
// C versija (Stage 9) - rankinis lifecycle
...
// C++ "v0" - automatinis lifecycle
{
    Stack stack;  // konstruktorius!
    stack.push(42);
    // ... destruktorius automatiškai!
}  // ← čia stack sunaikinama AUTOMATIŠKAI
```

**Studentai:** "Ooooh, kompiliatorius už mane!" → **RAII įsimintinumas**

---

## P14 KULMINACIJA (FULL CIRCLE)

```cpp
// Priedas 10D rodė: void* generics C kalboje - pain!
// C++ sprendimas:

template<typename T>
class Stack {
    std::unique_ptr<T[]> data;  // Smart pointer!
    size_t capacity;
    size_t top;
public:
    // Rule of Zero - viskas default!
...
// O STL?
std::stack<int> stl;  // Jūsų Stack + profesionalumas!
```

**Studentai:** "Ahaa, visa evoliucija iki `std::stack`!" → **Closure**

---

## RYŠYS SU DSA KURSU (lygiagrečiu)

| DSA kursas | OOP C++ kursas |
|---|---|
| **Stack ADT** (teorija, operacijos) | **Stack implementacija** (C→C++ evoliucija) |
| **Algoritmai** (rekursija su stack) | **RAII, copy/move** (kaip valdyti resursus) |
| **Duomenų struktūros** (array, linked list) | **Kaip realizuoti** (`CLASS`, `templates`) |

**Skirtumas:**  
- DSA: mokosi **"kas"** ir **"kodėl"**  
- OOP C++: mokosi **"kaip"** ir **"kodėl taip geriau"**
